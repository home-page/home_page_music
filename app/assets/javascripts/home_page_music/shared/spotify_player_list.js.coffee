...
<div class="modal-header">
  ...
  <h4 class="modal-title" id="myModalLabel">Play #{$(this).data('model')}</h4>
</div>
...
""" 
    ...
    $.each $('.spotify_player_list_item'), (index, element) =>
      ...
      key = "#{$(element).data('model')}:#{$(element).data('artist-name').toLowerCase()};#{$(element).data('name').toLowerCase()}"
  ...
  bulkRequest: (musicResources, models) -> 
    ...
    $.ajax(
      ...
      $.each data, (index, element) =>
        listItemIndexes = window.spotify_player_list.resourceToIndex["#{model}:#{element['artist_name'].toLowerCase()};#{element['name'].toLowerCase()}"]
      ...
      $.each $('.spotify_player_list_item'), (index, element) =>
        if $(element).data('model') == window.spotify_player_list.model
          listItemIndexes = window.spotify_player_list.resourceToIndex["#{model}:#{$(element).data('artist-name').toLowerCase()};#{$(element).data('name').toLowerCase()}"]
